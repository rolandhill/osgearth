...
MPTerrainEngineNode::MarkBoundingTiles(TileNode* tilenode, Side side, unsigned int tilesPerLod0Tile, unsigned int numTilesX0, unsigned int numTilesY0)
{
    ...
    osg::ref_ptr<TilePagedLOD> root = static_cast<TilePagedLOD*>( _terrain->getChild(0) ); // Root TilePagedLOD
    osg::ref_ptr<osg::Group> group = static_cast<osg::Group*>( root->getChild(index) ); // LOD0 TileGroup or Group
